...
makeSubmitJobResult = function(status, batch.job.id, msg,  ...) {
  ...
  structure(list(
    status=status, batch.job.id=batch.job.id, msg=msg),
...
#' @param killJob [\code{function(reg, batch.job.id)}]\cr
...
#' @seealso \link{makeClusterFunctionsLocal}, \link{makeClusterFunctionsSSH}, \link{makeClusterFunctionsTorque} 
makeClusterFunctions = function(name, submitJob, killJob, listJobs, class, ...) {
  ...
  if (!is.null(killJob))
    checkArg(killJob, "function", formals=c("reg", "batch.job.id"))
  if (!is.null(listJobs))
    checkArg(listJobs, "function", formals=c("reg"))
  res = structure(list(
    name=name, submitJob=submitJob, killJob=killJob, listJobs=listJobs),
