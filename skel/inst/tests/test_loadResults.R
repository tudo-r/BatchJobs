...
test_that("loadResults", {
  ...
  submitJobs(reg)
  expect_equal(names(loadResults(reg, use.names = TRUE, use.alias = FALSE)), as.character(1:26))
  expect_equal(names(loadResults(reg, use.names = TRUE, use.alias = TRUE)), letters)
})
